...
async def _(event: GroupMessageEvent, reg_group: Tuple[Any, ...] = RegexGroup()):
    ...
    if rank_data := await ChatHistory.get_group_msg_rank(
        ...
        for uid, num in rank_data:
            ...
            num_str += f"\t{num}\n\n"
            idx += 1
        name_img = await text2image(name.strip(), padding=10, color="#f9f6f2", font="HWZhongSong.ttf")
        num_img = await text2image(num_str.strip(), padding=10, color="#f9f6f2", font="HWZhongSong.ttf")
        if not date_scope:
