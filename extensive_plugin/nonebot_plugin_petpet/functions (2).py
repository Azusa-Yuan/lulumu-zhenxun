...
def rbq(user: UserInfo = User(), arg: str = Arg()):
    img = user.img.convert("RGBA").resize_width(500)
    img_w, img_h = img.size
    frame = BuildImage.new("RGBA", (600, img_h + 230), (255, 255, 255))
    ...
    name = (arg if arg else "") or user.name or 'ta'
    text = f"请问你们看到{name}了吗?"
    try:
        frame.draw_text(
            (20, 0, 540, 110), text, max_fontsize=60, min_fontsize=25, weight="bold"
        )
    except ValueError:
        return NAME_TOO_LONG
...
def budong(img: BuildImage = UserImg(), arg=NoArg()):
    frame = load_image("budong/0.png")
    img = img.convert("RGBA").resize((155, 155), keep_ratio=True)
