...
gpjk = on_command('监控', aliases={'jk'}, priority=5, permission=SUPERUSER)
show = on_command('看股票', aliases={'kgp'}, priority=5, permission=SUPERUSER)
stop = on_command("stop", priority=5, permission=SUPERUSER)
...
async def gpjk_handle(bot: Bot, event: Event):
    ...
    if len(temp) > 1:
        ...
        if is_num(temp[1]):
            bound = float(temp[1])
        if bound < 0:
            await gpjk.finish(message=Message("第三个位置请输入大于等于0的数~"))
    call_name = temp[0].strip()
    # resp = ef.stock.get_quote_history('gzmt')
    try:
        resp = ef.stock.get_quote_history(call_name, klt=1)
    ...
    if (name, UID) not in is_doing.keys():
        is_doing[(name, UID)] = 1
    else:
    ...
    while True:
        ...
        if len(res) == 0:
            res = ef.stock.get_quote_history(name, klt=1)['开盘']
        l = len(res) - 1
        if l == prel:
            cnt += change_time
        else:
        ...
        if abs(dis - pre) > bound:
            if dis > pre:
                if dis >= 4:
                    await gpjk.send(
                        message=Message(
                            f'【涨】{name} 净值现为 {now:.2f}，涨跌幅为 {dis:+.2f}%，虎年吃大肉！[CQ:face,id=320]'))
                elif dis >= 0:
                    await gpjk.send(
                        message=Message(f'【涨】{name} 净值现为 {now:.2f}，涨跌幅为 {dis:+.2f}%，较上次更新上涨，步步高升！'))
                else:
                    await gpjk.send(
                        message=Message(f'【涨】{name} 净值现为 {now:.2f}，涨跌幅为 {dis:+.2f}%，较上次更新上涨，拨云见日！'))
            else:
                if dis >= 0:
                    await gpjk.send(
                        message=Message(f'【跌】{name} 净值现为 {now:.2f}，涨跌幅为 {dis:+.2f}%，较上次更新下跌，再接再厉！'))
                else:
                    await gpjk.send(
                        message=Message(f'【跌】{name} 净值现为 {now:.2f}，涨跌幅为 {dis:+.2f}%，较上次更新下跌，来日方长！'))
            pre = dis
